        ...
        <div class="hidden md:block w-1/2 bg-primary py-10 px-10">
          <svg id="a87032b8-5b37-4b7e-a4d9-4dbfbe394641" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"
            ...
            <path
              ...
              transform="translate(-227.576 -76.46149)" fill="#F13130" />
        ...
        <div class="w-full md:w-1/2 py-14 px-10 md:px-14">
          ...
          <div>
            ...
            <div class="flex -mx-3">
              <div class="w-full px-3 mb-5">
                <button
                  class="block w-full max-w-xs mx-auto bg-primary hover:opacity-80 text-white rounded-lg px-3 py-3 font-bold"
                  :disabled="btnDisabled" @click="login">
                  登录
                </button>
              </div>
            </div>
...
const login = function () {
  ...
  api.post('/admin/login', form).then((res) => {
    if (res.data.code === 20000) {
      sessionStorage.setItem('token', res.data.data.token)
      localStorage.setItem('mobile', res.data.data.user.mobile)
      sessionStorage.setItem('user', JSON.stringify(res.data.data.user))
      ElMessage({type: 'success', message: `欢迎你，${res.data.data.user.name}`})
      router.push('/')
    }
    if (res.data.code !== 20000) {
